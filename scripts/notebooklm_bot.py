...
def _run_automation(topic_title: str, safe_name: str, output_path: Path, description: str = "") -> None:
    ...
    with sync_playwright() as p:
        # Clean up stale lock files from crashed previous sessions
        # These prevent Chrome from opening the profile
        for lock_file in ["SingletonLock", "SingletonCookie", "SingletonSocket"]:
            lock_path = Path(BROWSER_DATA_DIR) / lock_file
        ...
        context = p.chromium.launch_persistent_context(
            ...
            channel="chrome",  # Use system Chrome (more stable)
            headless=False,
