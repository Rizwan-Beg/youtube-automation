...
def create_thumbnail(topic_title: str, safe_name: str) -> Path:
    ...
    # Build the prompt
    prompt = (
        f"Create a highly engaging minimal cinematic professinalism YouTube thumbnail for the topic: '{topic_title}'. "
        "Style: high CTR, emotionally compelling, designed to maximize clicks. "
        "Include symbolic elements related to the topic "
        "Composition: rule of thirds, large central subject, depth and cinematic lighting, "
        "Style Options: semi-realistic or stylized illustration (YouTube viral style), minimal. "
        "Add subtle motion cues (arrows, glow, particles, perspective distortion) to imply movement. "
        "Background: clean but dynamic, not cluttered. Focus attention on subject. "
        "Lighting: cinematic, high dynamic range, sharp details. "
        "Quality: professional minimilistic YouTube thumbnail style. "
    )
   # "Color Psychology: use high-contrast colors (yellow, red, green) to trigger attention. "
    try:
